...
- Each configuration has a **target** (Default / Erlang / JavaScript). *Default* lets `gleam.toml`
  decide. Pick Erlang or JavaScript to force one and validate only that toolchain.
- `gleam test` output is a plain console. gleeunit prints passing tests as bare dots and has no
